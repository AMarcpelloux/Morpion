...
public class ConfigFileLoader {
	...
	/*
	 * Ajout d'une fonction pour écrire dans le fichier de configuration des modèles.
	 */
	public void writeConfigFile(String file,Config configTable[]) {
		try {
			File f = new File(file);
			FileWriter fw = new FileWriter(f);
			for(int i=0;i<configTable.length;i++) {
				fw.write(configTable[i].level+":"+
						configTable[i].hiddenLayerSize+":"+
						configTable[i].learningRate+":"+
						configTable[i].numberOfhiddenLayers+"\n"
						);
			}
			fw.close();
		}catch(Exception e) {
